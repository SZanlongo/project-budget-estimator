...
<tr>
	 ...
	 <td id="categories">
		<h2>Add <?php echo $sel_project_category['description']; ?> product.</h2>
                ...
                <form action="ProCategDetController.php?project_category_id=<?php echo
		...
		<?php	// create links to bit on product			
			$result_set = mysql_query($query, $connection);			
			while($row = mysql_fetch_array($result_set)){ echo "<a href='create_bit.php?proj_categ_detail=".urlencode($row['project_category_detail_id'])."'>".$row["item"] ."</a><br>";}	
		?> 	
	</td>		
</tr>
</table>
